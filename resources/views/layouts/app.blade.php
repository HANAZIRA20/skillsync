...
<head>
    ...
    <title>@yield('title', 'SkillSync') — AI-Powered Career Platform</title>
    <meta name="description" content="@yield('description', 'Platform marketplace AI yang menghubungkan mahasiswa dengan UMKM untuk proyek nyata.')">
    ...
    <aside class="w-64 bg-gray-900 border-r border-gray-800 flex flex-col fixed h-full z-40 overflow-y-auto">
        ...
        <div class="p-4 border-b border-gray-800">
            <div class="flex items-center space-x-3">
                <img src="{{ Auth::user()->avatar_url }}" class="w-10 h-10 rounded-full ring-2 ring-primary-600" alt="Avatar">
                <div class="min-w-0">
                    <p class="text-sm font-semibold text-white truncate">{{ Auth::user()->name }}</p>
                    <span class="text-xs px-2 py-0.5 rounded-full
                        {{ Auth::user()->role === 'mahasiswa' ? 'bg-primary-900 text-primary-300' : '' }}
                        {{ Auth::user()->role === 'client' ? 'bg-emerald-900 text-emerald-300' : '' }}
                        {{ Auth::user()->role === 'admin' ? 'bg-red-900 text-red-300' : '' }}
                    ">
        ...
        <nav class="flex-1 p-4 space-y-1">
            @if(Auth::user()->isMahasiswa())
                ...
                <a href="{{ route('student.dashboard') }}" class="sidebar-link flex items-center space-x-3 px-3 py-2.5 rounded-lg text-gray-300 hover:text-white {{ request()->routeIs('student.dashboard') ? 'active text-white' : '' }}">
                    <i class="fas fa-home w-4"></i><span>Dashboard</span>
                </a>
                <a href="{{ route('student.krs') }}" class="sidebar-link flex items-center space-x-3 px-3 py-2.5 rounded-lg text-gray-300 hover:text-white {{ request()->routeIs('student.krs*') ? 'active text-white' : '' }}">
                    <i class="fas fa-brain w-4"></i><span>Upload KRS</span>
                    @if(Auth::user()->student?->krs_status === 'not_uploaded')
                        <span class="ml-auto bg-amber-500 text-xs px-1.5 py-0.5 rounded-full text-black font-bold">!</span>
                    @endif
                </a>
                <a href="{{ route('student.portfolio') }}" class="sidebar-link flex items-center space-x-3 px-3 py-2.5 rounded-lg text-gray-300 hover:text-white {{ request()->routeIs('student.portfolio') ? 'active text-white' : '' }}">
                    <i class="fas fa-briefcase w-4"></i><span>Portfolio</span>
                </a>
                <a href="{{ route('student.projects') }}" class="sidebar-link flex items-center space-x-3 px-3 py-2.5 rounded-lg text-gray-300 hover:text-white {{ request()->routeIs('student.projects') ? 'active text-white' : '' }}">
            ...
            @elseif(Auth::user()->isClient())
                ...
                <a href="{{ route('client.dashboard') }}" class="sidebar-link flex items-center space-x-3 px-3 py-2.5 rounded-lg text-gray-300 hover:text-white {{ request()->routeIs('client.dashboard') ? 'active text-white' : '' }}">
                    <i class="fas fa-home w-4"></i><span>Dashboard</span>
                </a>
                <a href="{{ route('client.create-project') }}" class="sidebar-link flex items-center space-x-3 px-3 py-2.5 rounded-lg text-gray-300 hover:text-white {{ request()->routeIs('client.create-project') ? 'active text-white' : '' }}">
                    <i class="fas fa-plus-circle w-4"></i><span>Buat Proyek</span>
                </a>
                <a href="{{ route('client.projects') }}" class="sidebar-link flex items-center space-x-3 px-3 py-2.5 rounded-lg text-gray-300 hover:text-white {{ request()->routeIs('client.projects') ? 'active text-white' : '' }}">
            ...
            @elseif(Auth::user()->isAdmin())
                ...
                <a href="{{ route('admin.dashboard') }}" class="sidebar-link flex items-center space-x-3 px-3 py-2.5 rounded-lg text-gray-300 hover:text-white {{ request()->routeIs('admin.dashboard') ? 'active text-white' : '' }}">
                    <i class="fas fa-shield-alt w-4"></i><span>Dashboard Admin</span>
                </a>
                <a href="{{ route('analytics.dashboard') }}" class="sidebar-link flex items-center space-x-3 px-3 py-2.5 rounded-lg text-gray-300 hover:text-white {{ request()->routeIs('analytics.*') ? 'active text-white' : '' }}">
    ...
    <div class="ml-64 flex-1 flex flex-col min-h-screen">
        ...
        <header class="bg-gray-900/80 backdrop-blur-md border-b border-gray-800 px-8 py-4 flex items-center justify-between sticky top-0 z-30">
            ...
            <div class="flex items-center space-x-4">
                ...
                <img src="{{ Auth::user()->avatar_url }}" class="w-9 h-9 rounded-full ring-2 ring-primary-700" alt="Avatar">
            </div>
        </header>
